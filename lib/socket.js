...
const ATTACH_KEY_RE = /\.lack-proxy\d+\.h$/;
let index = 0;
...
socketProto.connect = function(...args) {
  ...
  startProxy(options, (tempPort) => {
    ...
    if (ATTACH_KEY_RE.test(host)) {
      ...
      options.attachData = attachDataCache.get(dataKey);
      options.attachHeaders = attachDataCache.get(key);
      attachDataCache.del(key);
      attachDataCache.del(dataKey);
    }
...
const getCacheKey = (host) => {
  let dataKey;
  let headersKey;
  if (ATTACH_KEY_RE.test(host)) {
    headersKey = RegExp['$&'];
    dataKey = headersKey.slice(0, -2);
  } else {
    dataKey = `.lack-proxy${`${Date.now()}`.slice(-6)}${index++}`;
    headersKey = `${dataKey}.h`;
    host = `${host}${headersKey}`;
  }
  return { host, dataKey, headersKey };
};
...
const attachData = (options, data, headers) => {
  ...
  if (typeof options === 'string') {
    cacheInfo = getCacheKey(options);
    options = cacheInfo.host;
  } else {
    options = Object.assign({}, options);
    cacheInfo = getCacheKey(options.host);
    options.host = cacheInfo.host;
  }
  if (data != null) {
    attachDataCache.set(cacheInfo.dataKey, data);
  }
  if (headers) {
    attachDataCache.set(cacheInfo.headersKey, headers);
  }
  return options;
};
