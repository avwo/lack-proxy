...
const { HTTP_REQ_FLAG, formatPatterns, checkPatterns } = require('./util');
...
const getProxy = (options, isHttps) => {
  ...
  if (typeof globalProxy === 'function') {
    proxy = globalProxy(Object.keys({}, options));
    if (!proxy || typeof proxy === 'function') {
      return;
    }
  ...
  if (!proxy.headers.host && /^([.\w-]+)(:\d+)?$/.test(host)) {
    const port = options.port || (RegExp.$2 && RegExp.$2.substring(1));
    proxy.headers.host = `${RegExp.$1}:${port || (isHttps ? 443 : 80)}`;
  }
  return proxy;
};
