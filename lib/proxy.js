...
const formatProxy = (proxy) => {
  ...
  return { host, port, headers, filterRequest: typeof filterRequest === 'function' ? filterRequest : null };
};
...
const createConnection = (options) => {
  ...
  headers.host = path;
  if (!headers['x-whistle-policy'] && options.isSocket !== 2) {
    headers['x-whistle-policy'] = 'tunnel';
  }
  if (!headers['x-lack-proxy-proto']) {
    headers['x-lack-proxy-proto'] = 'socket';
  }
  const proxyOpts = {
    [HTTP_REQ_FLAG]: true,
    method: 'CONNECT',
