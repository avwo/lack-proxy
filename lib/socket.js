...
const handleSocket = (socket) => {
  ...
  const key = `${socket.remotePort}:${tempPort}`;
...
const startProxy = (options, callback) => {
  ...
  const proxyOpts = getProxy(options);
  if (!proxyOpts || !proxyOpts.socket || (proxyOpts.filterRequest && !proxyOpts.filterRequest(options, true))) {
    return callback();
  }
...
socketProto.connect = function(...args) {
  ...
  startProxy(options, (proxyOpts) => {
    ...
    connect.call(this, tempProxyOpts, () => {
      ...
      options.proxy = proxyOpts.socket;
